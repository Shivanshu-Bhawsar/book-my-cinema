...
exports.updateScreen = async (req, res) => {
  try {
    ...
    const lastRegularSeat = await Seat.find({ seatType: regular.name }).sort({ seatNumber: -1 }).limit(1);
    const lastBolconySeat = await Seat.find({ seatType: bolcony.name }).sort({ seatNumber: -1 }).limit(1);
    const lastVipSeat = await Seat.find({ seatType: vip.name }).sort({ seatNumber: -1 }).limit(1);

    const lastRegularSeatNumber = lastRegularSeat.length > 0 ? lastRegularSeat[0].seatNumber : 0;
    const lastBolconySeatNumber = lastBolconySeat.length > 0 ? lastBolconySeat[0].seatNumber : 0;
    const lastVipSeatNumber = lastVipSeat.length > 0 ? lastVipSeat[0].seatNumber : 0;
