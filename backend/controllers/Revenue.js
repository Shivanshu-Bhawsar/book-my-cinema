...
exports.getAdminRevenueDetails = async (req, res) => {
  try {
    const { userId, startDate="2024-11-01", endDate="2024-12-27" } = req.body;
...
exports.getRevenueByCityId = async (req, res) => {
  try {
    const { cityId, startDate="2024-11-01", endDate="2024-12-27" } = req.body;

    const cityData = await City.findById(cityId);

      const cinemas = await Cinema.find({ cityId }).populate(
        "screens"
      );
