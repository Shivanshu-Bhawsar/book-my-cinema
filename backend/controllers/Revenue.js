...
exports.getAdminRevenueDetails = async (req, res) => {
  try {
    ...
    const adminDetails = await User.findById(userId).select('-password -booking');
    if (!adminDetails) {
    ...
    const revenueDetails = await Promise.all(
      cities.map(async (city) => {
        ...
        const cityRevenue = await cinemas.reduce(async (accPromise, cinema) => {
          ...
          const cinemaRevenue = await shows.reduce(async (showAccPromise, show) => {
  ...
  try {
    ...
    const cinemaDetails = await Promise.all(
      cinemas.map(async (cinema) => {
        ...
        const adminDetails = await User.findById(cinema.adminDetailes).select('-password -booking');
